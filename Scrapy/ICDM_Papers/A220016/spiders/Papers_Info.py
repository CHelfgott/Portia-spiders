...
class PapersInfo(BasePortiaSpider):
    ...
    def parse_item(self, response):
        loader = ItemLoader(item=AuditingBlackBoxModelsForIndirectInfluenceItem(),
                            selector=None,
                            response=response)

        loader.add_css('Title', 'h1.document-title > span[ng-bind-html="vm.displayDocTitle"]::text')
        ...
        loader.add_value('IEEE Keywords', ieee_kw_selector.extract())
        loader.add_value('Author Keywords', author_kw_selector.extract())
        ...
        for author in author_selector:
          author_loader = ItemLoader(item=AuthorItem())#, output_processor=TakeFirst())
          author_loader.add_value('Author', author.css('a > span[ng-bind-html="::author.name"]::text').extract_first())
          author_loader.add_value('Affiliation', author.css('a::attr(qtip-text)').extract_first())
          loader.add_value('Authors', author_loader.load_item())
